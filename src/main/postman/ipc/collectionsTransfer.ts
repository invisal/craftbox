...
const SUPPORTED_SCHEMAS_MESSAGE = 'Craftbox supports Postman Collection Format v2.0 and v2.1 (.json exports from Postman).';
...
export function registerCollectionTransferHandlers(): void {
  ...
  ipcMain.handle('collections:importFromFile', async (event): Promise<ImportCollectionResult> => {
    ...
    if (!isPostmanCollectionFile(parsed)) {
      return { ok: false, error: `File is not a recognized Postman Collection export. ${SUPPORTED_SCHEMAS_MESSAGE}` };
    }
