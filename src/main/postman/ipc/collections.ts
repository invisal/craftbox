...
export function registerCollectionHandlers(): void {
  ...
  ipcMain.handle('collections:createFolder', async (_event, payload: CreateFolderPayload): Promise<WsAckResult> => {
    const collections = await readCollections();
    const target = collections.find((c) => c.id === payload.collectionId);
    if (!target) return { ok: false, error: 'Collection not found.' };
  ...
  ipcMain.handle('collections:renameFolder', async (_event, payload: RenameFolderPayload): Promise<WsAckResult> => {
    const collections = await readCollections();
    const target = collections.find((c) => c.id === payload.collectionId);
    if (!target) return { ok: false, error: 'Collection not found.' };
  ...
  ipcMain.handle('collections:deleteFolder', async (_event, payload: DeleteFolderPayload): Promise<WsAckResult> => {
    const collections = await readCollections();
    const target = collections.find((c) => c.id === payload.collectionId);
    if (!target) return { ok: false, error: 'Collection not found.' };
  ...
  ipcMain.handle('collections:moveRequest', async (_event, payload: MoveRequestPayload): Promise<WsAckResult> => {
    const collections = await readCollections();
    const target = collections.find((c) => c.id === payload.collectionId);
    if (!target) return { ok: false, error: 'Collection not found.' };
  ...
  ipcMain.handle('collections:moveFolder', async (_event, payload: MoveFolderPayload): Promise<WsAckResult> => {
    const collections = await readCollections();
    const target = collections.find((c) => c.id === payload.collectionId);
    if (!target) return { ok: false, error: 'Collection not found.' };
    const folder = findFolder(target, payload.folderId);
    if (!folder) return { ok: false, error: 'Folder not found.' };
    if (payload.targetParentFolderId && isFolderOrDescendant(folder, payload.targetParentFolderId)) {
      return { ok: false, error: "Can't move a folder into itself or one of its own subfolders." };
    }
