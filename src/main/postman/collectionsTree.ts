...
export function findContainerOfRequest(container: RequestContainer, requestId: string): RequestContainer | null {
...
export function findParentOfFolder(container: RequestContainer, folderId: string): RequestContainer | null {
...
export function resolveContainer(collection: Collection, folderId: string | null | undefined): RequestContainer | null {
...
export function countRequestsRecursive(container: RequestContainer): number {
  return container.requests.length + container.folders.reduce((sum, f) => sum + countRequestsRecursive(f), 0);
}
