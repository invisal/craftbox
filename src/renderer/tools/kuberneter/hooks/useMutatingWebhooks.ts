...
export function useMutatingWebhooks(enabled: boolean) {
  const transform = useMemo(
    () => (items: K8sResource[]) => {
      return items.map((item) => {
        const mutatingItem = item as unknown as {
          apiVersion?: string;
        ...
        const name = mutatingItem.metadata?.name || '';
        ...
        const creationTimestamp = mutatingItem.metadata?.creationTimestamp || '';

        return {
          id: name,
          name,
          labels: mutatingItem.metadata?.labels,
          annotations: mutatingItem.metadata?.annotations,
