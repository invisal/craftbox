...
interface ResourceQuotasTableProps {
  filteredData: ResourceQuotaData[];
  selectedIds: Set<string>;
  onSelectAll: (checked: boolean) => void;
  onSelectRow: (id: string, checked: boolean) => void;
  onSelectResourceQuota: (rq: ResourceQuotaData) => void;
  selectedResourceQuotaId?: string;
}

export const ResourceQuotasTable: React.FC<ResourceQuotasTableProps> = ({
  filteredData,
  selectedIds,
  onSelectAll,
  onSelectRow,
  onSelectResourceQuota,
  selectedResourceQuotaId
}) => {
  const columns = useMemo<Column<ResourceQuotaData>[]>(
    () => [
      {
        key: 'select',
        header: (
  ...
  return (
    <KubeTable
      ...
      variant="standard"
      className="flex-1"
      onRowClick={(row) => onSelectResourceQuota(row)}
      selectedRowKey={selectedResourceQuotaId}
      emptyMessage="No resource quotas match the search filters."
    />
  );
};
