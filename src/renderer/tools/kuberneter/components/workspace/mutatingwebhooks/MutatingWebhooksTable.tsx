...
interface MutatingWebhooksTableProps {
  filteredData: MutatingWebhookConfigurationData[];
  selectedIds: Set<string>;
  onSelectAll: (checked: boolean) => void;
  onSelectRow: (id: string, checked: boolean) => void;
  onSelectWebhook: (webhook: MutatingWebhookConfigurationData) => void;
  selectedWebhookId?: string;
}

export const MutatingWebhooksTable: React.FC<MutatingWebhooksTableProps> = ({
  filteredData,
  selectedIds,
  onSelectAll,
  onSelectRow,
  ...
  const columns = useMemo<Column<MutatingWebhookConfigurationData>[]>(
    () => [
      {
        key: 'select',
        header: (
  ...
  return (
    <KubeTable
      ...
      emptyMessage="No Mutating Webhook Configurations match the search filters."
    />
  );
};
