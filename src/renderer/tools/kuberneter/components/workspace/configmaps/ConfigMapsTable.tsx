...
interface ConfigMapsTableProps {
  filteredData: ConfigMapData[];
  selectedIds: Set<string>;
  onSelectAll: (checked: boolean) => void;
  onSelectRow: (id: string, checked: boolean) => void;
  onSelectConfigMap: (cm: ConfigMapData) => void;
  selectedConfigMapId?: string;
}
...
export const ConfigMapsTable: React.FC<ConfigMapsTableProps> = ({
      ...
      },
      {
        ...
        render: (row) => {
          ...
          return (
            <span
              className={cn(
                'font-mono text-[11px] truncate block max-w-[450px]',
  ...
  return (
    <KubeTable
      ...
      emptyMessage="No config maps match the search filters."
    />
  );
};
