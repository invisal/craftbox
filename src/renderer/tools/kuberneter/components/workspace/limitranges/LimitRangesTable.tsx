...
interface LimitRangesTableProps {
  filteredData: LimitRangeData[];
  selectedIds: Set<string>;
  onSelectAll: (checked: boolean) => void;
  onSelectRow: (id: string, checked: boolean) => void;
  onSelectLimitRange: (lr: LimitRangeData) => void;
  selectedLimitRangeId?: string;
}

export const LimitRangesTable: React.FC<LimitRangesTableProps> = ({
  filteredData,
  selectedIds,
  onSelectAll,
  onSelectRow,
  onSelectLimitRange,
  selectedLimitRangeId
}) => {
  const columns = useMemo<Column<LimitRangeData>[]>(
    () => [
      {
        key: 'select',
        header: (
  ...
  return (
    <KubeTable
      ...
      emptyMessage="No limit ranges match the search filters."
    />
  );
};
