...
export const PostmanSidebar: React.FC = () => {
  ...
  useEffect(() => {
    // Newly loaded collections default to expanded (folders inside default to collapsed).
    setExpanded((prev) => {
  ...
  const handleImportCollection = async (): Promise<void> => {
    ...
    if (!result.ok) {
      ...
      const versionLabel =
        result.schemaVersion && result.schemaVersion !== 'unknown' ? ` (Postman Collection v${result.schemaVersion})` : '';
      setStatusMessage({
        type: 'success',
        ...
        <div className="flex flex-col gap-0.5">
          {collections.map((collection) => (
            <CollectionItem
              ...
              onMoveRequest={(requestId, targetFolderId) => moveRequest(collection.id, requestId, targetFolderId)}
              onNewRequest={(folderId) => openNewRequestInFolder(collection.id, folderId)}
              onCreateFolder={(parentFolderId, name) => createFolder(collection.id, parentFolderId, name)}
...
const MoveToFolderPopover: React.FC<MoveToFolderPopoverProps> = ({ folders, excludeId, title, onSelect }) => {
      ...
      <Popover.Portal>
        <Popover.Positioner sideOffset={6} align="start" className="z-50">
          ...
          >
            ...
            {options.length === 0 && <div className="px-2 py-1 text-zinc-600 italic">No other folders</div>}
          </Popover.Popup>
        </Popover.Positioner>
      </Popover.Portal>
    </Popover.Root>
...
const RequestItem: React.FC<RequestItemProps> = ({
  ...
  if (isRenaming) {
    return (
      <div style={style} className="flex items-center gap-2 p-1.5 rounded border border-accent bg-editor-bg">
