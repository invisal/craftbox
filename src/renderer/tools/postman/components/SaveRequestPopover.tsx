...
export const SaveRequestPopover: React.FC<SaveRequestPopoverProps> = ({
  ...
  const canConfirm = name.trim().length > 0 && (isCreatingCollection ? newCollectionName.trim().length > 0 : collectionId);
      ...
      <Popover.Portal>
        <Popover.Positioner sideOffset={8} align="end" className="z-50">
          <Popover.Popup className="bg-sidebar-bg border border-border-dark rounded-lg shadow-xl p-3 w-72 flex flex-col gap-3 text-xs outline-none">
            <div className="font-bold text-zinc-300 uppercase tracking-wider text-[10px]">Save Request</div>
