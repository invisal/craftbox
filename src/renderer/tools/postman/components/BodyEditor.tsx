...
import type { HttpBodyType, KeyValuePair } from '../../../../preload/postman/types';
...
export const BodyEditor: React.FC<BodyEditorProps> = ({ value, onChange, bodyType, variables, placeholder }) => {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="relative flex h-40 bg-editor-bg border border-border-dark rounded overflow-hidden focus-within:border-accent">
        <div ref={gutterRef} className="w-9 shrink-0 overflow-hidden py-2 border-r border-border-dark select-none">
          {Array.from({ length: lineCount }, (_, i) => (
            <div key={i} style={{ height: LINE_HEIGHT }} className="px-2 text-right text-[11px] leading-5 text-zinc-600">
              {i + 1}
            </div>
          ))}
        </div>
