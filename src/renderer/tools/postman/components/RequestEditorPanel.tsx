  ...
  return (
    ...
    >
      ...
      <Tabs.Panel value="params" className="max-h-40 overflow-auto pr-1">
        <KeyValueEditor rows={params} onUpdate={onUpdateParam} onRemove={onRemoveParam} keyPlaceholder="Param" />
      </Tabs.Panel>
      ...
      <Tabs.Panel value="body" className="flex flex-col gap-2">
        <div className="flex gap-3 text-[11px]">
          {BODY_TYPES.map((bt) => (
            <label key={bt.value} className="flex items-center gap-1.5 cursor-pointer text-zinc-400">
              <input
                type="radio"
                name="body-type"
