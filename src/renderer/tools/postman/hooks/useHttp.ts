...
import type { HttpBodyType, HttpMethod, HttpResponsePayload } from '../../../../preload/postman/types';
...
export function useHttp(tabId: string): UseHttpResult {
  ...
  const setMethod = useCallback((method: HttpMethod) => setState((prev) => ({ ...prev, method })), [setState]);

  const setUrl = useCallback(
    (url: string) => setState((prev) => ({ ...prev, url, params: mergeParamsFromUrl(url, prev.params) })),
    [setState]
  );

  const setBodyType = useCallback((bodyType: HttpBodyType) => setState((prev) => ({ ...prev, bodyType })), [setState]);
  const setBody = useCallback((body: string) => setState((prev) => ({ ...prev, body })), [setState]);
  ...
  const removeHeaderRow = useCallback(
    (id: string) => setState((prev) => ({ ...prev, headers: withTrailingRow(prev.headers.filter((row) => row.id !== id)) })),
    [setState]
  );
  ...
  return { state, setMethod, setUrl, setBodyType, setBody, updateHeaderRow, removeHeaderRow, updateParamRow, removeParamRow, send };
}
