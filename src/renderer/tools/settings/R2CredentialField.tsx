...
export function R2CredentialField() {
  ...
  return (
    <div className="max-w-md">
      <div className="flex items-center justify-between mb-1">
        <h2 className="text-sm font-medium">Cloudflare R2</h2>
        {status !== 'loading' && (
          <span
            className={cn(
      ...
      <p className="text-xs text-text-dim mb-4">
        Lets File Explorer browse the R2 buckets this token can see. All four fields are required
        together.
      </p>
      ...
      {status === 'configured' ? (
        ...
        <div className="flex flex-col gap-3">
          ...
          <Field label="R2 Access Key ID">
            <Input size="sm" value={accessKeyId} onChange={(e) => setAccessKeyId(e.target.value)} />
          </Field>
