...
export function ScreenRecordingPermissionBanner(): JSX.Element | null {
  ...
  useEffect(() => {
    window.screenStudio?.permissions.getScreenRecordingStatus().then(setStatus);
  }, []);
    ...
    <div className="flex items-start gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4">
      ...
      <div className="flex flex-1 flex-col gap-2">
        ...
        <p className="text-xs text-amber-200/70">
          ...
          in System Settings, then <strong>fully quit and reopen ScreenStudio</strong> -- Electron
          won't pick up the change while it's still running.
        </p>
        <div>
          <Button
            variant="secondary"
            onClick={() => window.screenStudio?.permissions.openScreenRecordingSettings()}
          >
